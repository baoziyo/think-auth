...
class Auth
{
     ...
     */
    public function check($name, $user, $type = 1, $mode = 'url', $relation = 'or')
    {
        if (!$this->config['auth_on']) {
            return true;
        }
        // 获取用户需要验证的所有有效规则列表
        $authList = $this->getAuthList($user, $type);
        if (is_string($name)) {
    ...
    public function getGroups($user)
    {
        ...
        $user_groups = db($auth_group)->where('id', $user['groupId'])->find();
        ...
        $user_groups['groupId'] = $user_groups['id'];
     ...
     * @return array
     */
    public function getAuthList($user, $type)
    {
        ...
        if (2 == $this->config['auth_type'] && Session::has('_auth_list_'.$user['id'].$t)) {
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) {
                //根据condition进行验证
                $user = $this->getUserInfo($user['id']); //获取用户信息,一维数组
